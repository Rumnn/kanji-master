...
export default function ITVocabQuiz() {
  ...
  const startQuiz = async () => {
    ...
    try {
      ...
      const { data } = await axios.get(
        `/api/it-vocab/quiz/generate?count=${packSize}`,
        config
      );
      ...
      setError(err.response?.data?.message || 'Không thể tải câu hỏi từ server. Có thể database chưa đủ từ vựng.');
    } finally {
      setLoading(false);
    }
  };
    ...
    if (user?.token) {
      ...
      try {
        ...
        await axios.post('/api/history', {
          quizName: `Trắc nghiệm IT Vocab (${questions.length} câu)`,
          score,
    ...
    return (
      <div className="max-w-3xl mx-auto animate-fade-in-up">
        <div className="text-center mb-10">
          <h1 className="text-4xl md:text-5xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-blue-500 to-indigo-600 mb-3">
            💻 Trắc Nghiệm Từ Vựng IT
          </h1>
          <p className="text-gray-500 text-lg font-medium">Luyện tập từ vựng chuyên ngành Công nghệ thông tin</p>
        </div>
        ...
        <div className="space-y-8">
          ...
          <div className="flex justify-center flex-wrap gap-4">
            ...
            <button
              onClick={startQuiz}
              disabled={loading}
              className="px-12 py-4 bg-gradient-to-r from-blue-500 to-indigo-600 text-white rounded-2xl font-bold text-lg shadow-lg hover:shadow-xl hover:shadow-blue-200 transform hover:-translate-y-1 transition-all duration-200 disabled:opacity-60 flex-1 sm:flex-none"
            >
        ...
        <div className="flex flex-col items-center">
          ...
          <div className="flex items-start gap-6 w-full mb-8">
            ...
            <div className="flex-1 bg-white/90 backdrop-blur-xl rounded-3xl shadow-xl p-8 border border-white text-center">
              <div className="text-6xl sm:text-7xl font-black text-gray-800 jp-text mb-4 drop-shadow-md">
          ...
          <div className="w-full grid grid-cols-1 sm:grid-cols-2 gap-4">
            {currentQuestion.choices.map((choice, idx) => (
              <ChoiceButton
                ...
                isJapanese={false} // choices are vietnamese
              />
            ))}
          </div>
          ...
          {isRevealed && selectedAnswer === null && (
            <div className="mt-6 p-4 bg-amber-50 rounded-2xl border border-amber-200 text-center">
              <p className="text-amber-700 font-bold">⏰ Hết giờ! Đáp án đúng: <span className="text-amber-900">{currentQuestion.correctAnswer}</span></p>
            </div>
          )}
        </div>
      </div>
    ...
    return (
      <div className="max-w-2xl mx-auto animate-fade-in-up">
        <div className="bg-white/80 backdrop-blur-xl rounded-[2rem] shadow-2xl p-12 text-center border border-white/40">
          ...
          <p className="text-gray-500 font-medium mb-8">Trắc nghiệm IT Vocab • {questions.length} câu</p>

          <div className="text-7xl font-black text-transparent bg-clip-text bg-gradient-to-br from-blue-500 to-indigo-600 mb-2">
            {score} <span className="text-4xl text-gray-400">/ {questions.length}</span>
          </div>
          <p className="text-xl text-gray-500 font-medium mb-10">
            Tỉ lệ đúng: <span className="text-indigo-600 font-bold">{percentage}%</span>
          </p>
            ...
            <div className="space-y-2 max-h-60 overflow-y-auto text-left">
              {questions.map((q, i) => (
                <div key={i} className={`flex items-center gap-3 p-3 rounded-xl text-sm ${answeredCorrectly[i] ? 'bg-emerald-50' : 'bg-rose-50'}`}>
                  <span className={`w-7 h-7 rounded-full flex items-center justify-center text-white text-xs font-bold shrink-0 ${answeredCorrectly[i] ? 'bg-emerald-500' : 'bg-rose-500'}`}>
                    {answeredCorrectly[i] ? '✓' : '✗'}
                  </span>
          ...
          <div className="flex gap-4 justify-center flex-wrap">
            <button
              ...
              className="px-8 py-4 bg-gradient-to-r from-blue-500 to-indigo-600 text-white rounded-xl font-bold hover:shadow-lg hover:shadow-blue-200 transform hover:-translate-y-1 transition-all duration-200"
            >
              🔄 Chơi lại
            </button>
            <button
              onClick={() => navigate('/leaderboard')}
              className="px-8 py-4 bg-gradient-to-r from-amber-400 to-amber-500 text-white rounded-xl font-bold hover:shadow-lg hover:shadow-amber-200 transform hover:-translate-y-1 transition-all duration-200"
            >
              🏆 Xếp hạng
            </button>
            <button
